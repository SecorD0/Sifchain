...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about a SifChain node"
		echo
		...
		echo -e "https://github.com/SecorD0/SifChain/blob/main/node_info.sh - script URL"
...
main() {
	...
	# Send Pull request with new texts to add a language - https://github.com/SecorD0/SifChain/blob/main/node_info.sh
	#elif [ "$language" = ".." ]; then
	else
...
"$balance" 2>/dev/null
		...
		if [ "$catching_up" = "true" ]; then
			local current_block=`wget -qO- "https://api-sifchain.cosmostation.io/v1/status" | jq -r ".block_height"`
